...
FDR_plot <- ggplot(all_summaries_df, aes(x=FoldChange, y=FDR, color=Method, group=Method)) +
  ...
  xlab("Fold Change") + ylab("False Discovery Rate") + theme_bw() + 
  theme(text=element_text(size=14), legend.position = "bottom")

Power_plot <- ggplot(all_summaries_df, aes(x=FoldChange, y=Power, color=Method, group=Method)) +
  geom_point(size=1.2, alpha=0.8) + geom_line(linewidth=1, alpha=0.7) + 
  scale_y_continuous(limits=c(0, 1))+
  facet_grid(cols=vars(Direction)) + scale_color_brewer(palette="Dark2")+
  xlab("Fold Change") + ylab("Power") + theme_bw() + 
  theme(text=element_text(size=14), legend.position = "bottom")
