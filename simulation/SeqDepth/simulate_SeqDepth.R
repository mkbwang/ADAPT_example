...
settings_df <- expand.grid(nSample=100,
                           nTaxa=500,
                           seqdepth_mean=c(5e3, 1e4, 2e4, 4e4),
                           propDA=0.1,
...
maaslin2_output <-invisible(Maaslin2(input_data=count_df, input_metadata=metadata_df,
                            output = file.path(folder, 'SeqDepth', 'maaslin2_output', sprintf("seed_%d", myseed)),
                            min_prevalence=0.05,
...
saveRDS(performance_summary, file=file.path(folder, 'SeqDepth', 'experiments', output_filename))
