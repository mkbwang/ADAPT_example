...
for (choice in choices){
  ...
  results_summary$SeqDepth <- as.character(settings_df$Depth[choice])
...
all_summaries_df$SeqDepth <- factor(all_summaries_df$SeqDepth, 
                                  levels=c("10000", "20000", "50000"))
all_summaries_df$Direction <- factor(all_summaries_df$Direction, 
                                     levels=c("balanced", "unbalanced"))


FDR_plot <- ggplot(all_summaries_df, aes(x=SeqDepth, y=FDR, color=Method, group=Method)) +
...
Power_plot <- ggplot(all_summaries_df, aes(x=SeqDepth, y=Power, color=Method, group=Method)) +
  geom_point(size=1.2, alpha=0.8) + geom_line(linewidth=1, alpha=0.7) + 
  scale_y_continuous(limits=c(0, 1))+
  facet_grid(cols=vars(Direction)) + scale_color_brewer(palette="Dark2")+
  xlab("SeqDepth") + ylab("Power") + theme_bw() + 
  theme(text=element_text(size=14), legend.position = "bottom")
