...
FDR_plot_1 <- ggplot(subset_summaries_df_1, aes(x=Seqdepth, y=FDR, color=isADAPT, group=Method)) +
...
Power_plot_1 <- ggplot(subset_summaries_df_1, aes(x=Seqdepth, y=Power, color=isADAPT, group=Method)) +
...
FDR_plot_2 <- ggplot(subset_summaries_df_2, aes(x=Seqdepth, y=FDR, color=isADAPT, group=Method)) +
  geom_point(size=1.4, alpha=0.8) + geom_line(linewidth=0.8, alpha=0.7) + 
  geom_hline(yintercept=0.05, linetype="dotted", color="red") +
  scale_y_continuous(limits=c(0, 0.15))+
  scale_color_manual(values=manual_color)+
...
Power_plot_2 <- ggplot(subset_summaries_df_2, aes(x=Seqdepth, y=Power, color=isADAPT, group=Method)) +
  geom_point(size=1.2, alpha=0.8) + geom_line(linewidth=0.8, alpha=0.7) + 
  scale_y_continuous(limits=c(0, 1), breaks=seq(0, 1, 0.1))+
  scale_color_manual(values=manual_color)+
  xlab("Average Library Size") + ylab("Power") + theme_bw() + 
