...
old_POLDA_folder <- "/home/wangmk/MDAWG/POLDA/simulation/POLDA"
new_POLDA_folder <- "/home/wangmk/MDAWG/POLDA/simulation/new_POLDA"

# polda result summary
old_polda_performance <- data.frame(ID=seq(1, 100),
                                AUROC=rep(0, 100),
                                FDR=rep(0, 100),
                                Power=rep(0, 100))
for (j in 1:100){
  polda_result <- read.csv(file.path(old_POLDA_folder, 
  ...
  old_polda_performance$AUROC[j] <- polda_roc$auc
  DA_truth <- polda_result$logfold != 0
  polda_contingency <- table(DA_truth, polda_result$pval_adjust < 0.05)
  if (ncol(polda_contingency) == 2){
    old_polda_performance$FDR[j] <- polda_contingency[1, 2] / sum(polda_contingency[, 2])
    old_polda_performance$Power[j] <- polda_contingency[2, 2] / sum(polda_contingency[2, ]) 
  }
}
old_polda_performance$Method <- 'old_POLDA'


new_polda_performance <- data.frame(ID=seq(1, 100),
                                    N_Reftaxa = rep(0, 100),
                                    N_FalseRef = rep(0, 100),
                                    AUROC=rep(0, 100),
                                    FDR=rep(0, 100),
                                    Power=rep(0, 100))
# polda_result <- read.csv(file.path(new_POLDA_folder, 
#                                    sprintf('polda_simulation_result_%d.csv', 2)))
for (j in 1:100){
  polda_result <- read.csv(file.path(new_POLDA_folder, 
                                     sprintf('polda_simulation_result_%d.csv', j)))
  ## for those reference taxa, I put a placeholder of 1 as their p values
  polda_result$pval_adjust[polda_result$RefTaxa] <- 1
  polda_reftaxa <- polda_result %>% filter(RefTaxa)
  new_polda_performance$N_Reftaxa[j] <- nrow(polda_reftaxa)
  new_polda_performance$N_FalseRef[j] <- sum(polda_reftaxa$logfold != 0)
  polda_result_subset <- polda_result[!polda_result$RefTaxa, ]
  ## if there are still taxa whose p values are NA, then they are structural zeros
  polda_result$pval_adjust[is.na(polda_result$pval_adjust)] <- 0
  polda_roc <- roc(polda_result_subset$logfold!=0, 1-polda_result_subset$pval_adjust)
  new_polda_performance$AUROC[j] <- polda_roc$auc
  DA_truth <- polda_result$logfold != 0
  polda_contingency <- table(DA_truth, polda_result$pval_adjust < 0.05)
  if (ncol(polda_contingency) == 2){
    new_polda_performance$FDR[j] <- polda_contingency[1, 2] / sum(polda_contingency[, 2])
    new_polda_performance$Power[j] <- polda_contingency[2, 2] / sum(polda_contingency[2, ]) 
  }
}
new_polda_performance$Method <- 'new_POLDA'
...
auroc_combined <- cbind(aldex2_performance$AUROC,
                        ...
                        old_polda_performance$AUROC,
                        new_polda_performance$AUROC) |> as.data.frame()
colnames(auroc_combined) <- c("Aldex2", 'ANCOM', "ANCOMBC", "Old_POLDA", "New_POLDA")
...
combined_performance <- rbind(aldex2_performance, ancom_performance, ancombc_performance, 
                              old_polda_performance, new_polda_performance[, c(1,4,5,6,7)])
