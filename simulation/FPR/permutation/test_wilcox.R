...
wilcox_pvals <- rep(0, nrow(permuted_counts))
...
for (k in 1:nrow(permuted_counts)){
  ...
  wilcox_model <- wilcox.test(logratios ~ metadata$X)
  wilcox_pvals[k] <- wilcox_model$p.value
  
}


avg_5 <- mean(wilcox_pvals < 0.05)
avg_10 <- mean(wilcox_pvals < 0.1)
avg_20 <- mean(wilcox_pvals < 0.2)
avg_30 <- mean(wilcox_pvals < 0.3)

output <- sprintf("%d\t%.4f\t%.4f\t%.4f\t%.4f", j, avg_5, avg_10, avg_20, avg_30)
write(output, file=file.path(simulation_folder, "FPR", "permutation", "wilcox_FPR.txt"),append=T)
