...
# prevalences <- rowMeans(count_matrix != 0)
# 
# count_USA <- count_matrix[, !metadata$isUSA]
# prevalences_USA <- rowMeans(count_USA != 0)
# count_USA <- count_USA[prevalences_USA > 0.05, ]
...
# # permuted_counts <- truncate_counts(permuted_counts, targetzeros = 0.6)
# prevalences <- rowMeans(permuted_counts > 0)
# permuted_counts <- permuted_counts[prevalences > 0.1, ]

CR_result <- count_ratio(count_data=count_matrix,
                              metadata=metadata,
                              covar="X",
                             ratio_model="loglogistic", pen=0.5)
pvals <- CR_result$pval
...
write(output, file=file.path(simulation_folder, "FPR", "permutation", "loglogistic_FPR.txt"), append=T)
