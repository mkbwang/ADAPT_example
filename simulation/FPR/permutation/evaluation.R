...
lognormal_performance <- read.table(file.path(simulation_folder, "FPR", "permutation", "lognormal_FPR.txt"),
                                 header=F, sep='\t')
colnames(lognormal_performance) <- c("Run", "FPR_5", "FPR_10", "FPR_20", "FPR_30")
lognormal_performance$Model <- "Lognormal"
...
all_performances <- rbind(cox_performance,  lognormal_performance, loglogistic_performance) # lm_performance, wilcox_performance)
all_performances$Model <- factor(all_performances$Model, levels=c("Cox", "Lognormal", "loglogistic"))
...
cutoff_20 <- ggplot(all_performances, aes(x=Model, y=FPR_20)) + geom_violin() +  geom_boxplot(width=0.1) + 
  ...
  geom_hline(yintercept=0.2, linetype="dashed", color="red", linewidth=1)+ylim(0, 0.4)+
  theme_bw() + theme(plot.title = element_text(hjust = 0.5), text=element_text(size=14))
