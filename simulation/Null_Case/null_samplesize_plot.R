...
subset_summaries <- all_summaries_df %>% filter(nTaxa == "1000") %>% filter(!is.na(FPR))
subset_summaries$FPRLabel <- ""
subset_summaries$FWERLabel <- ""
subset_summaries$FPRLabel[subset_summaries$FPR > 0.1 & subset_summaries$nSample == "200"] <- 
  subset_summaries$Method[subset_summaries$FPR > 0.1 & subset_summaries$nSample == "200"]
subset_summaries$FPRLabel[subset_summaries$nSample == "200" & subset_summaries$Method=="ADAPT"] <- "ADAPT"

subset_summaries$FWERLabel[subset_summaries$FWER > 0.1 & subset_summaries$nSample == "200"] <- 
  subset_summaries$Method[subset_summaries$FWER > 0.1 & subset_summaries$nSample == "200"]
subset_summaries$FWERLabel[subset_summaries$nSample == "200" & subset_summaries$Method=="ADAPT"] <- "ADAPT"

subset_summaries$nSample <- factor(subset_summaries$nSample, 
                                      levels=c("50", "100", "200"))
...
FPR_plot <- ggplot(subset_summaries, aes(x=nSample, y=FPR, color=isADAPT, group=Method)) +
  ...
  xlab("Sample Size") + ylab("Type I Error") + theme_bw() + 
...
FWER_plot <- ggplot(subset_summaries, aes(x=nSample, y=FWER, color=isADAPT, group=Method)) +
  geom_point(size=1.4, alpha=0.8) + geom_line(linewidth=0.8, alpha=0.7, linetype="dashed") + 
  geom_hline(yintercept=0.05, linetype="dashed", color="red") +
