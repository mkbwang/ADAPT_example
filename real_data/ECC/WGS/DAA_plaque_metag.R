...
dacomp_summary$DACOMP = grepl("FALSE<=TRUE", Direction)
dacomp_summary$DACOMP = 2*(dacomp_summary$DACOMP - 0.5)
dacomp_summary$Taxa <- str_replace_all(dacomp_summary$Taxa,"\\."," ")
...
begin <- proc.time()
ancom_output <- ancom(data=phy_metag_plaque, p_adj_method="BH", prv_cut=0.05,
                      main_var="CaseStatus", n_cl=4)
...
# ANCOMBC
ancombc_output <- ancombc2(phy_metag_plaque, fix_formula = 'CaseStatus', p_adj_method='BH', global=T,
                           group='CaseStatus', struc_zero=FALSE, prv_cut=0.05, n_cl=4)

ancombc_result <- ancombc_output$res
ancombc_summary <- ancombc_result %>% filter(diff_CaseStatuscontrol) %>%
  mutate(Taxa=taxon, ANCOMBC=lfc_CaseStatuscontrol < 0, .keep="none")
...
# aggregate outputs
library(purrr)
combined_result <- purrr::reduce(list(adapt_summary, aldex_summary, maaslin2_summary, dacomp_summary,
