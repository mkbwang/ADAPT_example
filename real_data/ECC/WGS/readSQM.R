...
saliva_object = loadSQM(file.path(folder, "WGS", "metasqueeze", "saliva_assembly"), 
                        engine = 'data.table')
                        
species_counts <- saliva_object$taxa$species$abund
taxonomies <- saliva_object$misc$tax_names_long$species
taxonomy_table <- sapply(taxonomies, 
                         function(longname) strsplit(longname, split=";")[[1]] )
taxonomy_table <- t(taxonomy_table)
taxon_filter <- taxonomy_table[, 1] == "k_Bacteria"
...
saliva_metadata <- metag_metashort %>% filter(SampleType == "Saliva")
saliva_metadata$iscase <- saliva_metadata$CaseStatus == "Case"
rownames(saliva_metadata) <- saliva_metadata$OriginalID

phy_metag_saliva <- phyloseq(otu_table(species_counts_subset, taxa_are_rows = T),
                             tax_table(taxonomy_table),
                             sample_data(saliva_metadata))

saveRDS(phy_metag_saliva, file=file.path(folder, "WGS", "phy_metag_saliva.rds"))
