...
phy_metag_saliva <- readRDS(file.path(folder, "phyloseq", "phy_metag_saliva.rds"))


count_table <- otu_table(phy_metag_saliva)@.Data
prevalences <- rowMeans(count_table != 0)
count_table_subset <- count_table[prevalences > 0.05, ]
taxa_names <- rownames(count_table_subset)
metadata <- data.frame(sample_data(phy_metag_saliva))
...
# ANCOMBC
library(ANCOMBC)
ancombc_output <- ancombc2(phy_metag_saliva, fix_formula='CaseStatus',
                           p_adj_method='BH',
...
taxonomy_table <- tax_table(phy_metag_saliva)@.Data
selected_taxonomies <- taxonomy_table[output_df$Taxon, ] |> as.data.frame()
output_df <- cbind(output_df, selected_taxonomies)


write.csv(output_df, file.path(folder, "method_comparison_WGS_saliva.csv"),
          row.names=T)
write.csv(adapt_result_df, file.path(folder, "ADAPT_WGS_saliva_result.csv"),
          row.names=F)
