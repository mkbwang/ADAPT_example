...
metadata <- sample_data(phy_asv_month24)
...
# ANCOMBC
library(ANCOMBC)
ancombc_output <- ancombc2(phy_asv_month24, fix_formula='CaseEver',
                           p_adj_method='BH',
...
write.csv(output_df, file.path(folder, "method_comparison_24month.csv"),
          row.names=T)
write.csv(adapt_result_df, file.path(folder, "ADAPT_24month_result.csv"),
          row.names=F)
